...
namespace HamiltonEngine::Physics 
{
	...
	enum class EulerMode 
	{
		...
		KineticOnly,
		PotentialOnly,
		EulerA, //kinetic first, potential second
		EulerB  //potential first, kinetic second
	};
	...
	template<EulerMode Mode>
	void EulerSystem(const MassComponent& MassC,
		PositionComponent& PosC,
		LinearMomentumComponent& LinMomC,
		float PotentialWeight = 1.0f,
		float KineticWeight = 1.0f,
		float Dt = Globals::PhysicsTickLength)
	{
		if constexpr (Mode == EulerMode::KineticOnly)
		{
			EulerModeBKineticOnlySystem(MassC, PosC, LinMomC, KineticWeight, Dt);
		}
		else if constexpr (Mode == EulerMode::PotentialOnly)
		{
			EulerModeBPotentialOnlySystem(MassC, PosC, LinMomC, PotentialWeight, Dt);
		}
		else if constexpr (Mode == EulerMode::EulerA)
		{
			EulerModeBKineticOnlySystem(MassC, PosC, LinMomC, KineticWeight, Dt);
			EulerModeBPotentialOnlySystem(MassC, PosC, LinMomC, PotentialWeight, Dt);
		}
		else if constexpr (Mode == EulerMode::EulerB)
		{
			EulerModeBPotentialOnlySystem(MassC, PosC, LinMomC, PotentialWeight, Dt);
			EulerModeBKineticOnlySystem(MassC, PosC, LinMomC, KineticWeight, Dt);
		}
